...
uint32_t FM::query(uint32_t i) {
	int32_t dist = 0, rank;
	uint8_t c;
	while (!sampled->access(i)) {
	    c = T_bwt->access(i);
	    rank = T_bwt->rank(c,i)-1;
	    i = C[c]+rank; // LF-mapping
	    ++dist;
	}
	return suffixes[sampled->rank1(i)-1]+dist;
}

uint32_t*
FM::locate(uint8_t* pattern,uint32_t m,uint32_t* matches, struct sa_rlbwt *sarl) {
    uint32_t* locations;
    ...
    if (sp<=ep) {
        ...
        *matches = ep-sp+1;
        ...
        while (i<=ep) {
	    if (sarl) {
		locations[locate] = query_sa_rlbwt(sarl, i - 1);
	    } else {
            	j=i,dist=0;
